...
const fetchHeroData = async ({ queryKey }: any) => {
  const [, apiEndPoint, id, params = {}] = queryKey
      ...
      <CharacterDiv>
        <CharacterInfo>
          ...
          <Description>
            <h6>Description</h6>
            <p>
              {charData?.description !== '' ? charData?.description : 'No description available.'}
            </p>
          </Description>
      ...
      charData?.events.items.length > 0 ? (
        <RelatedWork>
          {charData?.comics.items.length > 0 ? (
            <RelatedWorkList data={charData.comics.items} title="Comics" />
          ) : (
            ''
          )}

          {charData?.series.items.length > 0 ? (
            <RelatedWorkList data={charData.series.items} title="Series" />
          ) : (
            ''
          )}
          {charData?.events.items.length > 0 ? (
            <RelatedWorkList data={charData.events.items} title="Events" />
          ) : (
            ''
          )}
        </RelatedWork>
